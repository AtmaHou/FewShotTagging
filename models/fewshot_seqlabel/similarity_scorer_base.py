...
class MatchingSimilarityScorer(SimilarityScorerBase):
    ...
    def forward(
        ...
        test_mask = self.expand_it(test_output_mask, support_size).contiguous().view(batch_size * support_size,  test_len)
        support_mask = support_output_mask.contiguous().view(batch_size * support_size, support_len)
...
class ProtoWithLabelSimilarityScorer(SimilarityScorerBase):
    ...
    def remove_0(self, my_tensor):
        """
        Add a small value to avoid zero devision
        """
        return my_tensor + 0.0001


class TapNetSimilarityScorer(SimilarityScorerBase):
    def __init__(self, sim_func, num_tags, mlp_out_dim, random_init=False, random_init_r=1.0, mlp=False, emb_log=None,
                 tap_proto=False, tap_proto_r=1.0, proj_dim=0):
        super(TapNetSimilarityScorer, self).__init__(sim_func=sim_func, emb_log=emb_log)
        self.num_tags = num_tags
        self.random_init = random_init
        self.bert_emb_dim = 768
        if self.random_init:
            self.label_reps = torch.nn.Parameter(
                torch.nn.init.xavier_normal_(torch.randn((num_tags*3, self.bert_emb_dim))), requires_grad=True)
        self.mlp = mlp
    ...
    def forward(
        ...
        if label_reps is not None:
            if self.random_init:
                if no_pad_num_tags > len(self.label_reps):
                    raise NotImplementedError
                random_label_idxs = torch.randperm(len(self.label_reps))
                random_label_reps = self.label_reps[random_label_idxs[:no_pad_num_tags],:]
            ...
            random_label_idxs = torch.randperm(len(self.label_reps))
            random_label_reps = self.label_reps[random_label_idxs[:no_pad_num_tags], :]
            label_reps = random_label_reps.unsqueeze(0).repeat(batch_size, 1, 1).to(support_reps.device)
        ...
        # flatten dim mention of support size and batch size.
        # shape (batch_size * support_size, sent_len, emb_dim)
        support_reps = support_reps.view(-1, support_len, self.mlp_out_dim)
        ...
        sum_reps = torch.bmm(torch.transpose(support_targets, -1, -2), support_reps)
        # sum up tag emb over support set, shape (batch_size, num_tags, emd_dim)
        sum_reps = torch.sum(sum_reps.view(batch_size, support_size, num_tags, self.mlp_out_dim), dim=1)
        ...
        try:
            # torch version >= 1.2.0 has the batch process function
        ...
        # select the remain dimension of project space
        # shape (batch_size, emb_dim, D)
        if self.proj_dim:
            M = torch.stack([torch.transpose(vh[i][s_sum[i]:s_sum[i]+self.proj_dim].clone(), 0, 1) for i in range(batch_size)], dim=0)
        else:
