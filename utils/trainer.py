...
from utils.data_helper import PadCollate, FewShotDataset, SimilarLengthSampler
...
class TrainerBase:
    ...
    """
    def __init__(self, opt, optimizer, param_to_optimize, device, n_gpu, tester=None):
        """
        :param opt: args
        :param optimizer:
        ...
        self.n_gpu = n_gpu
        if 0 < opt.decay_lr < 1:
            self.scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=opt.decay_epoch_size, gamma=opt.decay_lr)
        else:
            self.scheduler = None
    ...
    def do_train(self, model, train_features, num_train_epochs,
        ...
        for epoch_id in trange(int(num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(data_loader, desc="Train-Batch Progress")):
                ...
                # loss, _ = self.do_forward(batch, model, epoch_id, step)
    ...
    def model_selection(self, model, best_score, dev_features, dev_id2label, test_features=None, test_id2label=None):
        ...
        # copy reusable params, for a different domain
        dev_model = self.tester.clone_model(model, dev_id2label, mat_type='dev')
        if self.opt.mask_transition:
        ...
        if dev_score > best_score:
            ...
            if test_features:
                ...
                test_model = self.tester.clone_model(model, test_id2label, mat_type='test')
                if self.opt.mask_transition:
...
class FewShotTrainer(TrainerBase):
    ...
    """
    def __init__(self, opt, optimizer, param_to_optimize, device, n_gpu, tester=None):
        super(FewShotTrainer, self).__init__(opt, optimizer, param_to_optimize, device, n_gpu, tester)
    ...
    def do_forward(self, batch, model, epoch_id, step):
        ...
        # Fix bert embedding for first 'fix_embed_epoch' epoch
        if self.opt.fix_embed_epoch > 1 and not self.opt.no_embedder_grad:
            if epoch_id < self.opt.fix_embed_epoch:
                model.no_embedder_grad = True
            else:
                model.no_embedder_grad = False
    ...
    def check_point_content(self, model):
        ...
        ret = {
            ...
            'opt': self.opt,
            'num_tags': sub_model.transition_scorer.num_tags if sub_model.transition_scorer else 1,
            'trans_r': sub_model.transition_scorer.r if sub_model.transition_scorer else -1,
        }
        return ret
    ...
    def clone_model(self, model):
        ...
        old_num_tags = len(self.get_value_from_order_dict(old_model.state_dict(), 'label_reps'))
        best_model = make_model(
            opt=old_model.opt,
            num_tags=old_model.transition_scorer.num_tags if old_model.transition_scorer else 1,
            trans_r=old_model.transition_scorer.r if old_model.transition_scorer else -1,
...
class SchemaFewShotTrainer(FewShotTrainer):
    def __init__(self, opt, optimizer, param_to_optimize, device, n_gpu, tester=None):
        super(SchemaFewShotTrainer, self).__init__(opt, optimizer, param_to_optimize, device, n_gpu, tester)
...
def prepare_optimizer(opt, model, num_train_features):
    num_train_steps = int(
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_to_optimize if not any(nd in n for nd in no_decay)], 'weight_decay_rate': 0.01},
        {'params': [p for n, p in param_to_optimize if any(nd in n for nd in no_decay)], 'weight_decay_rate': 0.0}
    ]
    ...
    return param_to_optimize, optimizer


def prepare_few_shot_optimizer(opt, model, num_train_features):
    """ allow different lr for back off trans """
    num_train_steps = int(
        num_train_features / opt.train_batch_size / opt.gradient_accumulation_steps * opt.num_train_epochs)
    if opt.fp16:
        param_to_optimize = [(n, param.clone().detach().to('cpu').float().requires_grad_())
        ...
        param_to_optimize = list(model.named_parameters())  # all parameter name and parameter
    no_decay = ['bias', 'gamma', 'beta']
    upper_structure_decay = ['backoff', 'scale_rate', 'f_theta', 'phi', 'start_reps', 'end_reps', 'biaffine']  # a set of parameter name subseuqence/mark
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_to_optimize if
                    not any(nd in n for nd in no_decay) and not any(nd in n for nd in upper_structure_decay)],
         'weight_decay_rate': 0.01, 'lr': opt.learning_rate},
        {'params': [p for n, p in param_to_optimize if
                    any(nd in n for nd in no_decay) and not any(nd in n for nd in upper_structure_decay)],
         'weight_decay_rate': 0.0, 'lr': opt.learning_rate},
        {'params': [p for n, p in param_to_optimize if
                    any(nd in n for nd in upper_structure_decay)],
         'weight_decay_rate': 0.1, 'lr': opt.upper_lr},
    ]
