...
def basic_args(parser):
    ...
    group.add_argument("--embedding_cache", type=str,
                       default='/users4/ythou/Projects/Homework/ComputationalSemantic/.word_vectors_cache',
    ...
    group.add_argument("-doft", "--do_overfit_test", default=False, action='store_true',
                       help="debug model, test/dev on train")
...
def preprocess_args(parser):
    ...
    group.add_argument("--sim_annotation", default='match', type=str,
                       choices=['match', 'BI-match', 'attention'],
                       help="Define annotation of token similarity")
    group.add_argument("--bio_text_omit", default=False, action="store_true", help="whether omit the BIO text or not")
    return parser
...
def train_args(parser):
    ...
    group.add_argument("--convergence_window", default=30000, type=int,
    ...
    group.add_argument('--decay_lr', type=float, default=-1, help="decay Learn Rate,eg: 0.1 set < 0 to disable this")
    group.add_argument('--decay_epoch_size', type=int, default=1, help="decay Learn Rate every decay_epoch_size")
    ...
    group.add_argument("--warmup_epoch", type=int, default=-1,
                       help="Num of epoch, when train model in two step. "
                            "Train model in two step: 1: fix bert part 2: train entire model"
                            "-1 to close two stage")
    ...
    group.add_argument("--fix_embed_epoch", default=-1, type=int, help="Fix embedding for first x epochs.")
...
def model_args(parser):
    ...
    group.add_argument("--context_emb", default='bert', type=str,
                       choices=['bert', 'elmo', 'glove', 'raw', 'sep_bert'],
    ...
    group.add_argument("--decoder", default='crf', type=str, choices=['crf', 'sms', 'rule'], help="decode method")
    # emission setting
    ...
    group.add_argument("--proj_dim", default=0, type=int,
                       help="project space dimension, `0` mean all remained dimension")
    ...
    group.add_argument('--backoff_init', default='rand', choices=['rand', 'fix', 'global_statistic'],
